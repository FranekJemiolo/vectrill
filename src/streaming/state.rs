...
impl WindowStateStore {
    ...
    pub fn get_or_create(
        ...
        if !self.states.contains_key(&key) {
            ...
            self.states
                .insert(key.clone(), WindowState::new(key.0.clone(), key.1.clone()));
        }
    ...
    pub fn remove_expired(&mut self, watermark: i64) -> Vec<WindowState> {
        ...
        self.states.retain(|(_window_key, _), state| {
    ...
    fn evict_oldest(&mut self) {
        if let Some(key) = self.states.keys().next().cloned() {
...
mod tests {
    ...
    fn test_remove_expired() {
        ...
        store
            .get_or_create(window_key1, None)
            .update_aggregate("value", 10.0);
        store
            .get_or_create(window_key2, None)
            .update_aggregate("value", 20.0);
