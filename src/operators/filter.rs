...
impl FilterOperator {
    ...
    pub fn from_expr(expr: &Expr, schema: arrow::datatypes::SchemaRef) -> Result<Self, VectrillError> {
    ...
    pub fn apply(&self, batch: &arrow::record_batch::RecordBatch) -> Result<arrow::record_batch::RecordBatch, VectrillError> {
        ...
        let mask_array = self.predicate.evaluate(batch)
            .map_err(|e| VectrillError::ExpressionError(e.to_string()))?;
        
        // Convert to boolean array
        let mask = mask_array.as_any()
            .downcast_ref::<arrow::array::BooleanArray>()
            .ok_or_else(|| VectrillError::ExpressionError("Predicate must return boolean array".to_string()))?;
        ...
        let filtered_batch = arrow::record_batch::RecordBatch::try_new(
            batch.schema(),
            filtered_columns,
...
fn filter_column(
    ...
    use arrow::array::{Array, PrimitiveArray, StringArray, BooleanArray};
    
    if indices.is_empty() {
        // Return empty array of same type
        return match column.data_type() {
            arrow::datatypes::DataType::Int64 => {
                Ok(Arc::new(PrimitiveArray::<arrow::datatypes::Int64Type>::from(vec![0i64; 0])))
            }
            arrow::datatypes::DataType::Utf8 => {
                Ok(Arc::new(StringArray::from(vec![""; 0])))
            }
            arrow::datatypes::DataType::Boolean => {
                Ok(Arc::new(BooleanArray::from(vec![false; 0])))
            }
...
mod tests {
    ...
    fn test_filter_boolean_predicate() {
        ...
        let batch = arrow::record_batch::RecordBatch::try_new(
            ...
            vec![
                Arc::new(Int64Array::from(vec![1, 2, 3, 4, 5])),
                Arc::new(arrow::array::BooleanArray::from(vec![true, false, true, false, true])),
            ],
        ).unwrap();
