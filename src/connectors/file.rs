...
impl FileConnector {
    ...
    pub fn new(path: PathBuf, format: FileFormat, schema: SchemaRef) -> Result<Self> {
        ...
        let reader = match format {
            FileFormat::Csv => {
                ...
                let headers = reader.headers().map_err(|e| VectrillError::Connector(format!("CSV error: {}", e)))?;
                for (i, header) in headers.iter().enumerate() {
                    if let Some(field) = schema.fields().get(i) {
                        if field.name() != header {
                            return Err(VectrillError::InvalidSchema(
                                format!("Header '{}' doesn't match schema field '{}'", header, field.name())
                            ));
                        }
                    }
                }
            ...
            FileFormat::Json => {
                return Err(VectrillError::Connector("JSON file connector not yet implemented".to_string()));
            }
            FileFormat::Parquet => {
                return Err(VectrillError::Connector("Parquet file connector not yet implemented".to_string()));
            }
        };
    ...
    fn read_csv_batch(&mut self) -> Option<Result<ArrowRecordBatch>> {
        ...
        for _ in 0..self.batch_size {
            match reader.read_record(&mut record) {
                ...
                Ok(false) => break,
                Err(e) => {
                    return Some(Err(VectrillError::Connector(format!("CSV read error: {}", e))));
                }
            }
        }
    ...
    fn records_to_batch(&self, records: &[csv::StringRecord]) -> Result<ArrowRecordBatch> {
        ...
        let mut builders: Vec<StringBuilder> = (0..num_columns)
...
impl Connector for FileConnector {
    async fn next_batch(&mut self) -> Option<Result<RecordBatch>> {
        ...
        match self.format {
            ...
            FileFormat::Json => {
                Some(Err(VectrillError::Connector("JSON file connector not yet implemented".to_string())))
            }
            FileFormat::Parquet => {
                Some(Err(VectrillError::Connector("Parquet file connector not yet implemented".to_string())))
            }
        }
    }
