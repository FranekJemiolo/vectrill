...
#[cfg(feature = "python")]
use super::arrow_bridge::{export_batch_to_python, pyany_to_record_batch};
#[cfg(feature = "python")]
use crate::sequencer::{LateDataPolicy, Ordering, Sequencer, SequencerConfig};
...
impl PySequencer {
        ...
        let sequencer_config =
            if let Some(config_dict) = config {
                ...
                if let Some(ordering_str) = config_dict.get_item("ordering")? {
                    let ordering =
                        match ordering_str.extract::<&str>()? {
    ...
    fn ingest(&mut self, batch: Bound<'_, PyAny>) -> PyResult<()> {
        ...
        self.inner.ingest(record_batch).map_err(|e| {
