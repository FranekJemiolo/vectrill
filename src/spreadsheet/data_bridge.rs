...
use arrow::array::{ArrayRef, StringArray, Float64Array, BooleanArray, Int64Array};
use arrow::record_batch::RecordBatch;
use std::sync::Arc;
use super::api::{SpreadsheetData, CellValue, DataType as SpreadsheetDataType};
...
impl DataBridge {
    ...
    pub fn spreadsheet_to_arrow(&self, data: &SpreadsheetData) -> Result<RecordBatch> {
        if data.headers.is_empty() || data.rows.is_empty() {
            return Err(VectrillError::InvalidData("Empty data provided".to_string()));
        }
        
        // Infer data types if not provided
        let column_types = if data.column_types.is_empty() || data.column_types.len() != data.headers.len() {
    ...
    fn infer_column_type(&self, data: &SpreadsheetData, col_idx: usize) -> Result<SpreadsheetDataType> {
    ...
    fn is_date_string(&self, s: &str) -> bool {
        ...
        s.to_lowercase().contains("am") || s.to_lowercase().contains("pm")
    }
    
    /// Create Arrow schema from headers and column types
    fn create_arrow_schema(&self, headers: &[String], column_types: &[SpreadsheetDataType]) -> Result<Arc<Schema>> {
    ...
    fn spreadsheet_to_arrow_type(&self, stype: &SpreadsheetDataType) -> Result<ArrowDataType> {
        match stype {
            ...
            SpreadsheetDataType::Date => Ok(ArrowDataType::Timestamp(arrow::datatypes::TimeUnit::Millisecond, None)),
    ...
    fn arrow_to_spreadsheet_type(&self, atype: &ArrowDataType) -> Result<SpreadsheetDataType> {
        match atype {
            ...
            ArrowDataType::Float64 | ArrowDataType::Float32 | ArrowDataType::Int64 | ArrowDataType::Int32 => Ok(SpreadsheetDataType::Number),
    ...
    fn create_arrow_arrays(&self, data: &SpreadsheetData, column_types: &[SpreadsheetDataType]) -> Result<Vec<ArrayRef>> {
    ...
    fn cell_value_to_timestamp(&self, value: &CellValue) -> Option<i64> {
        match value {
            CellValue::String(s) => {
                // Try to parse as timestamp (simple implementation)
                // In a real implementation, this would use a proper date parsing library
        ...
        match array.data_type() {
            ArrowDataType::Utf8 => {
                let string_array = array.as_any().downcast_ref::<StringArray>()
                    .ok_or_else(|| VectrillError::ArrowError("Failed to downcast to StringArray".to_string()))?;
                Ok(CellValue::String(string_array.value(row_idx).to_string()))
            }
            ArrowDataType::Float64 => {
                let float_array = array.as_any().downcast_ref::<Float64Array>()
                    .ok_or_else(|| VectrillError::ArrowError("Failed to downcast to Float64Array".to_string()))?;
                Ok(CellValue::Number(float_array.value(row_idx)))
            }
            ArrowDataType::Boolean => {
                let bool_array = array.as_any().downcast_ref::<BooleanArray>()
                    .ok_or_else(|| VectrillError::ArrowError("Failed to downcast to BooleanArray".to_string()))?;
                Ok(CellValue::Boolean(bool_array.value(row_idx)))
            }
            ArrowDataType::Timestamp(arrow::datatypes::TimeUnit::Millisecond, _) => {
                let timestamp_array = array.as_any().downcast_ref::<Int64Array>()
                    .ok_or_else(|| VectrillError::ArrowError("Failed to downcast to Int64Array".to_string()))?;
                Ok(CellValue::String(timestamp_array.value(row_idx).to_string()))
            }
            _ => {
...
mod tests {
    ...
    fn test_cell_type_inference() {
        ...
        assert!(matches!(bridge.infer_cell_type(&CellValue::Number(42.0)), SpreadsheetDataType::Number));
        assert!(matches!(bridge.infer_cell_type(&CellValue::Boolean(true)), SpreadsheetDataType::Boolean));
        assert!(matches!(bridge.infer_cell_type(&CellValue::Empty), SpreadsheetDataType::Empty));
        assert!(matches!(bridge.infer_cell_type(&CellValue::String("hello")), SpreadsheetDataType::String));
        assert!(matches!(bridge.infer_cell_type(&CellValue::String("42")), SpreadsheetDataType::Number));
    }
    ...
    fn test_value_conversions() {
        ...
        assert_eq!(bridge.cell_value_to_string(&CellValue::Number(42.5)), Some("42.5".to_string()));
        assert_eq!(bridge.cell_value_to_number(&CellValue::String("3.14")), Some(3.14));
        assert_eq!(bridge.cell_value_to_boolean(&CellValue::String("true")), Some(true));
        assert_eq!(bridge.cell_value_to_boolean(&CellValue::Number(1.0)), Some(true));
        assert_eq!(bridge.cell_value_to_boolean(&CellValue::Number(0.0)), Some(false));
    }
}
