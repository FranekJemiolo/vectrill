...
impl TemplateManager {
    ...
    pub fn create_pipeline_from_template(
        &self,
        template_name: &str,
        parameters: &HashMap<String, String>,
    ) -> Result<TransformationPipeline> {
        let template = self.get_template(template_name).ok_or_else(|| {
            VectrillError::InvalidConfig(format!("Template '{}' not found", template_name))
        })?;
    ...
    fn create_transformation_from_step(
        &self,
        step: &TemplateStep,
        parameters: &HashMap<String, String>,
    ...
    fn create_filter_transform(
        &self,
        step: &TemplateStep,
        parameters: &HashMap<String, String>,
    ) -> Result<Box<dyn Transformation>> {
        let _column = parameters
            .get("column")
            .or_else(|| step.parameters.get("column"))
            .ok_or_else(|| {
                VectrillError::InvalidConfig("Column parameter required for filter".to_string())
            })?;

        let default_operator = "equals".to_string();
        let operator_str = parameters
            .get("operator")
            .or_else(|| step.parameters.get("operator"))
            .unwrap_or(&default_operator);

        let _operator = match operator_str.as_str() {
        ...
        let value_str = parameters
            ...
            .ok_or_else(|| {
                VectrillError::InvalidConfig("Value parameter required for filter".to_string())
            })?;

        let _value = if let Ok(num) = value_str.parse::<f64>() {
        ...
        Err(VectrillError::NotImplemented(
            "Filter transformation creation not yet implemented".to_string(),
        ))
    }

    /// Create map transformation from step
    fn create_map_transform(
        &self,
        step: &TemplateStep,
        parameters: &HashMap<String, String>,
    ) -> Result<Box<dyn Transformation>> {
        let _column = parameters
            ...
            .or_else(|| step.parameters.get("column"))
            .ok_or_else(|| {
                VectrillError::InvalidConfig("Column parameter required for map".to_string())
            })?;

        let default_function = "identity".to_string();
        let function_str = parameters
            .get("function")
            .or_else(|| step.parameters.get("function"))
            .unwrap_or(&default_function);

        let _operation = match function_str.as_str() {
            "add" => {
        ...
        Err(VectrillError::NotImplemented(
            "Map transformation creation not yet implemented".to_string(),
        ))
    }
    ...
    fn create_standardize_text_template(&self) -> TransformationTemplate {
        ...
        parameters.insert(
            ...
            TemplateParameter {
                ...
                required: false,
                validation: Some(
                    "trim|lowercase|uppercase|remove_punctuation|normalize_spaces".to_string(),
                ),
            },
        );
