...
impl ComparisonOps {
    /// Equality comparison
    pub fn equal(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
    ...
    pub fn not_equal(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
        let len = left.len().max(right.len());
        let mut bool_array = Vec::with_capacity(len);
    ...
    pub fn less_than(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
        let left_ints = left.as_any().downcast_ref::<Int64Array>().ok_or(
            ExpressionError::TypeMismatch {
                expected: "Int64".to_string(),
                actual: format!("{:?}", left.data_type()),
            },
        )?;
        let right_ints = right.as_any().downcast_ref::<Int64Array>().ok_or(
            ExpressionError::TypeMismatch {
                expected: "Int64".to_string(),
    ...
    pub fn less_than_equal(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
        let left_ints = left.as_any().downcast_ref::<Int64Array>().ok_or(
            ExpressionError::TypeMismatch {
                expected: "Int64".to_string(),
                actual: format!("{:?}", left.data_type()),
    ...
    pub fn greater_than(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
        let left_ints = left.as_any().downcast_ref::<Int64Array>().ok_or(
            ExpressionError::TypeMismatch {
                expected: "Int64".to_string(),
                actual: format!("{:?}", left.data_type()),
    ...
    pub fn greater_than_equal(left: &ArrayRef, right: &ArrayRef) -> Result<Arc<dyn arrow::array::Array>, ExpressionError> {
        let left_ints = left.as_any().downcast_ref::<Int64Array>().ok_or(
            ExpressionError::TypeMismatch {
                expected: "Int64".to_string(),
                actual: format!("{:?}", left.data_type()),
