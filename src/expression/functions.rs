...
impl FunctionRegistry {
    ...
    pub fn register_function(
        ...
        if functions.contains_key(&name) {
            return Err(ExpressionError::UnsupportedExpression(
...
fn abs_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    if let Some(floats) = array.as_any().downcast_ref::<Float64Array>() {
        let result: Vec<f64> = floats.iter().map(|v| v.map(|x| x.abs()).unwrap_or(0.0)).collect();
        Ok(Arc::new(Float64Array::from(result)))
    } else if let Some(ints) = array.as_any().downcast_ref::<Int64Array>() {
        let result: Vec<i64> = ints.iter().map(|v| v.map(|x| x.abs()).unwrap_or(0)).collect();
...
fn sqrt_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let array = &args[0];
    let floats = array.as_any().downcast_ref::<Float64Array>().ok_or(
    ...
    let result: Vec<f64> = floats.iter().map(|v| v.map(|x| x.sqrt()).unwrap_or(0.0)).collect();
    Ok(Arc::new(Float64Array::from(result)))
}
...
fn pow_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let base_floats = base.as_any().downcast_ref::<Float64Array>().ok_or(
        ExpressionError::TypeMismatch {
            expected: "Float64".to_string(),
    ...
    let exp_floats = exp.as_any().downcast_ref::<Float64Array>().ok_or(
        ExpressionError::TypeMismatch {
            expected: "Float64".to_string(),
...
fn floor_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let result: Vec<f64> = floats.iter().map(|v| v.map(|x| x.floor()).unwrap_or(0.0)).collect();
    Ok(Arc::new(Float64Array::from(result)))
}
...
fn ceil_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let result: Vec<f64> = floats.iter().map(|v| v.map(|x| x.ceil()).unwrap_or(0.0)).collect();
    Ok(Arc::new(Float64Array::from(result)))
}
...
fn round_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let decimals = if args.len() == 2 {
        ...
        dec_array.and_then(|arr| if arr.len() > 0 { Some(arr.value(0)) } else { Some(0) }).unwrap_or(0)
    } else {
        0
    };
    ...
    let result: Vec<f64> = floats
        .iter()
        .map(|v| v.map(|x| (x * multiplier).round() / multiplier).unwrap_or(0.0))
        .collect();
...
fn length_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    if let Some(strings) = array.as_any().downcast_ref::<StringArray>() {
        let result: Vec<i64> = strings.iter().map(|s| s.map(|x| x.len() as i64).unwrap_or(0)).collect();
        Ok(Arc::new(Int64Array::from(result)))
    } else {
...
fn upper_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let array = &args[0];
    let strings = array.as_any().downcast_ref::<StringArray>().ok_or(
        ExpressionError::TypeMismatch {
    ...
    let result: Vec<Option<String>> = strings.iter().map(|s| s.map(|x| x.to_uppercase())).collect();
...
fn lower_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let result: Vec<Option<String>> = strings.iter().map(|s| s.map(|x| x.to_lowercase())).collect();
    Ok(Arc::new(StringArray::from(result)))
}
...
fn trim_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let result: Vec<Option<String>> = strings.iter().map(|s| s.map(|x| x.trim().to_string())).collect();
    Ok(Arc::new(StringArray::from(result)))
}
...
fn concat_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let string_arrays: Vec<&StringArray> = args
        .iter()
        .map(|arr| {
            arr.as_any().downcast_ref::<StringArray>().ok_or(
                ExpressionError::TypeMismatch {
                    expected: "Utf8".to_string(),
...
fn substring_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    let string_array = args[0].as_any().downcast_ref::<StringArray>().ok_or(
        ExpressionError::TypeMismatch {
            expected: "Utf8".to_string(),
    ...
    let start_array = args[1].as_any().downcast_ref::<Int64Array>().ok_or(
        ExpressionError::TypeMismatch {
    ...
    let length_array = args[2].as_any().downcast_ref::<Int64Array>().ok_or(
        ExpressionError::TypeMismatch {
            expected: "Int64".to_string(),
...
fn min_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    if let Some(floats) = array.as_any().downcast_ref::<Float64Array>() {
        let min = floats.iter().filter_map(|v| v).min_by(|a, b| a.partial_cmp(b).unwrap());
...
fn max_function(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    if let Some(floats) = array.as_any().downcast_ref::<Float64Array>() {
        let max = floats.iter().filter_map(|v| v).max_by(|a, b| a.partial_cmp(b).unwrap());
...
mod tests {
    ...
    fn test_custom_function_registration() {
        ...
        let custom_fn: FunctionSignature = |_args| {
            Ok(Arc::new(Float64Array::from(vec![42.0])))
        };
        ...
        assert!(registry.register_function("custom", custom_fn, metadata).is_ok());
