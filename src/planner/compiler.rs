...
use crate::planner::logical::{LogicalPlan, WindowSpec, WindowType, AggSpec, AggFunction};
...
impl PlanCompiler {
    ...
    pub fn compile(&self, logical_plan: &LogicalPlan) -> Result<PhysicalPlan, VectrillError> {
        match logical_plan {
            LogicalPlan::Source { name, attrs: _ } => {
                Ok(PhysicalPlan::ScanSource {
                    name: name.clone(),
                    attrs: HashMap::new(),
                })
            }
            ...
            PhysicalPlan::HashAggregate { input, keys, aggregations } => {
                ...
                for key in keys {
                    if input_schema.column_with_name(key).is_none() {
                        return Err(VectrillError::InvalidSchema(
                            format!("Group key '{}' not found in input schema", key)
                        ));
                    }
                }
                ...
                for agg in aggregations {
                    if input_schema.column_with_name(&agg.column).is_none() {
                        return Err(VectrillError::InvalidSchema(
                            format!("Aggregation column '{}' not found in input schema", agg.column)
                        ));
                    }
                }
            ...
            PhysicalPlan::Project { input, columns } => {
                ...
                for &col_idx in columns {
                    if col_idx >= input_schema.fields().len() {
                        return Err(VectrillError::InvalidSchema(
                            format!("Column index {} out of bounds for input schema with {} columns", 
                                col_idx, input_schema.fields().len())
                        ));
                    }
                }
            ...
            PhysicalPlan::Sort { input, sort_keys } => {
                ...
                for sort_key in sort_keys {
                    if input_schema.column_with_name(&sort_key.column).is_none() {
                        return Err(VectrillError::InvalidSchema(
                            format!("Sort key column '{}' not found in input schema", sort_key.column)
                        ));
                    }
                }
        ...
        match plan {
            PhysicalPlan::ScanSource { name, attrs } => {
                ...
                match name.as_str() {
                    ...
                    _ => Err(VectrillError::InvalidExpression(
                        format!("Unsupported source type: {}", name)
                    )),
                }
            }
            
            PhysicalPlan::Filter { input, .. } => {
                self.validate_operations(input)
            }
            
            PhysicalPlan::Map { input, .. } => {
                self.validate_operations(input)
            }
...
impl PythonDSLCompiler {
    ...
    pub fn compile(node: &serde_json::Value) -> Result<LogicalPlan, VectrillError> {
        ...
        match op {
            ...
            "filter" => {
                ...
                let expr_str = attrs.get("expr")
                    .ok_or_else(|| VectrillError::InvalidExpression("Filter missing 'expr'".to_string()))?;
            ...
            "map" => {
                ...
                let expr_str = attrs.get("expr")
                    .ok_or_else(|| VectrillError::InvalidExpression("Map missing 'expr'".to_string()))?;
            ...
            "group_by" => {
                ...
                let key_str = attrs.get("key")
                    .ok_or_else(|| VectrillError::InvalidExpression("GroupBy missing 'key'".to_string()))?;
            ...
            "window" => {
                ...
                let spec_str = attrs.get("spec")
                    .ok_or_else(|| VectrillError::InvalidExpression("Window missing 'spec'".to_string()))?;
            ...
            "agg" => {
                ...
                let spec_obj = node.get("attrs")
                    .and_then(|v| v.get("spec"))
                    .and_then(|v| v.as_object())
                    .ok_or_else(|| VectrillError::InvalidExpression("Aggregate missing 'spec'".to_string()))?;
    ...
    fn parse_window_spec(spec_str: &str) -> Result<WindowSpec, VectrillError> {
        ...
        if spec_str.starts_with("tumbling(") {
            let duration = spec_str
                ...
                .ok_or_else(|| VectrillError::InvalidExpression("Invalid tumbling window spec".to_string()))?;
            ...
            let inner = spec_str
                ...
                .and_then(|s| s.strip_suffix(')'))
                .ok_or_else(|| VectrillError::InvalidExpression("Invalid sliding window spec".to_string()))?;
            
            let parts: Vec<&str> = inner.split(',').collect();
            if parts.len() != 2 {
                return Err(VectrillError::InvalidExpression("Sliding window requires duration and slide".to_string()));
            }
            ...
            let timeout = spec_str
                ...
                .and_then(|s| s.strip_suffix(')'))
                .ok_or_else(|| VectrillError::InvalidExpression("Invalid session window spec".to_string()))?;
            ...
            Err(VectrillError::InvalidExpression(
                format!("Unknown window specification: {}", spec_str)
            ))
        }
    }
    ...
    fn parse_agg_function(fn_str: &str) -> Result<AggFunction, VectrillError> {
        match fn_str {
            ...
            _ => Err(VectrillError::InvalidExpression(
                format!("Unknown aggregation function: {}", fn_str)
            )),
        }
    }
}
