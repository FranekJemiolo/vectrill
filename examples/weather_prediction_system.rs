...
impl Transformation for WindChillCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        ...
        let wind_chill_values: Vec<Option<f64>> = temp_array
            ...
            .map(
                |(temp_opt, wind_speed_opt)| match (temp_opt, wind_speed_opt) {
                    (Some(temp), Some(wind_speed)) => {
                        Some(Self::calculate_wind_chill(temp, wind_speed))
                    }
                    _ => None,
                },
            )
...
impl StormRiskCalculator {
    ...
    fn calculate_storm_risk(
        ...
        (pressure_risk * 0.3 + precipitation_risk * 0.3 + wind_risk * 0.2 + cloud_risk * 0.2)
            * 100.0
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(batch_result) = source.next_batch().await {
        ...
        if let Some(is_extreme_array) = transformed_batch
            .column(15)
            .as_any()
