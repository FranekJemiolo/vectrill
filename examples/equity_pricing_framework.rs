...
fn ticks_to_record_batch(ticks: Vec<EquityTick>, schema: Arc<Schema>) -> Result<RecordBatch, VectrillError> {
    ...
    let spread_array = arrow::array::new_null_array(
        &arrow::datatypes::DataType::Float64, 
        ticks.len()
    );
    let mid_price_array = arrow::array::new_null_array(
        &arrow::datatypes::DataType::Float64, 
        ticks.len()
    );
    let price_change_array = arrow::array::new_null_array(
        &arrow::datatypes::DataType::Float64, 
        ticks.len()
    );
    let vwap_array = arrow::array::new_null_array(
        &arrow::datatypes::DataType::Float64, 
        ticks.len()
    );
...
impl Transformation for SpreadCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let bid_array = batch.column(4).as_any().downcast_ref::<Float64Array>().unwrap();
        let ask_array = batch.column(5).as_any().downcast_ref::<Float64Array>().unwrap();
...
impl Transformation for PriceChangeCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let symbol_array = batch.column(0).as_any().downcast_ref::<StringArray>().unwrap();
        let price_array = batch.column(2).as_any().downcast_ref::<Float64Array>().unwrap();
...
impl Transformation for VwapCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let symbol_array = batch.column(0).as_any().downcast_ref::<StringArray>().unwrap();
        let price_array = batch.column(2).as_any().downcast_ref::<Float64Array>().unwrap();
        let volume_array = batch.column(3).as_any().downcast_ref::<Int64Array>().unwrap();
        
        let vwap_values: Vec<Option<f64>> = symbol_array
            .iter()
            .zip(price_array.iter())
            .zip(volume_array.iter())
            .map(|((symbol_opt, price_opt), volume_opt)| match (symbol_opt, price_opt, volume_opt) {
                (Some(symbol), Some(price), Some(volume)) => {
                    let volume_f = volume as f64;
                    let cum_volume = self.cumulative_volume.entry(symbol.to_string()).or_insert(0.0);
                    let cum_value = self.cumulative_value.entry(symbol.to_string()).or_insert(0.0);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(batch_result) = source.next_batch().await {
        ...
        println!("🔄 Processing batch {} ({} rows)", batch_count, batch.num_rows());
