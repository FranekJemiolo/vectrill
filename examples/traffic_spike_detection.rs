...
use arrow::array::{Float64Array, Int64Array, StringArray, BooleanArray, Array};
use chrono::Utc;
...
fn generate_traffic_data(events_count: usize) -> Vec<TrafficEvent> {
    ...
    let dest_ips = vec![
        "8.8.8.8", "1.1.1.1", "208.67.222.222", "9.9.9.9", "149.112.112.112"
    ];
    ...
    for i in 0..events_count {
        ...
        let baseline = baseline_traffic.entry(key.clone()).or_insert(1000.0 + rand::random::<f64>() * 500.0);
...
fn traffic_to_record_batch(events: Vec<TrafficEvent>, schema: Arc<Schema>) -> Result<RecordBatch, VectrillError> {
...
impl Transformation for TrafficMetricsCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let bytes_array = batch.column(4).as_any().downcast_ref::<Int64Array>().unwrap();
        let packet_array = batch.column(5).as_any().downcast_ref::<Int64Array>().unwrap();
        let duration_array = batch.column(6).as_any().downcast_ref::<Int64Array>().unwrap();
        ...
        for i in 0..batch.num_rows() {
            if !bytes_array.is_null(i) && !packet_array.is_null(i) && !duration_array.is_null(i) {
                ...
                let bps = if duration_sec > 0.0 { bytes / duration_sec } else { 0.0 };
                let pps = if duration_sec > 0.0 { packets / duration_sec } else { 0.0 };
...
impl Transformation for TrafficRatioCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let source_ip_array = batch.column(0).as_any().downcast_ref::<StringArray>().unwrap();
        let dest_ip_array = batch.column(1).as_any().downcast_ref::<StringArray>().unwrap();
        let traffic_type_array = batch.column(9).as_any().downcast_ref::<StringArray>().unwrap();
        let bytes_per_sec_array = batch.column(10).as_any().downcast_ref::<Float64Array>().unwrap();
        
        let mut traffic_ratios = Vec::new();
        
        for i in 0..batch.num_rows() {
            if !source_ip_array.is_null(i) && !dest_ip_array.is_null(i) && 
               !traffic_type_array.is_null(i) && !bytes_per_sec_array.is_null(i) {
                ...
                let baseline = self.baseline_metrics.entry(key.clone()).or_insert(bytes_per_sec);
                *baseline = *baseline * 0.9 + bytes_per_sec * 0.1;  // Exponential moving average
                
                let ratio = if *baseline > 0.0 { bytes_per_sec / *baseline } else { 1.0 };
...
impl TrafficSpikeDetector {
    ...
    fn determine_anomaly_type(ratio: f64, bytes_per_sec: f64) -> &'static str {
        if ratio > 10.0 {
            ...
            "SMALL_SPIKE"
        } else if bytes_per_sec > 1000000.0 {  // > 1MB/s
            "HIGH_VOLUME"
        } else {
            "NORMAL"
        }
...
impl Transformation for TrafficSpikeDetector {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let traffic_ratio_array = batch.column(13).as_any().downcast_ref::<Float64Array>().unwrap();
        let bytes_per_sec_array = batch.column(10).as_any().downcast_ref::<Float64Array>().unwrap();
...
impl RiskAssessmentCalculator {
    ...
    fn assess_risk_level(
        ...
        // High volume traffic
        if bytes_per_sec > 10000000.0 {  // > 10MB/s
            risk_score += 30.0;
        } else if bytes_per_sec > 1000000.0 {  // > 1MB/s
            risk_score += 10.0;
        }
...
impl Transformation for RiskAssessmentCalculator {
    async fn apply(&mut self, batch: &RecordBatch) -> Result<RecordBatch, VectrillError> {
        let is_spike_array = batch.column(14).as_any().downcast_ref::<BooleanArray>().unwrap();
        let spike_severity_array = batch.column(15).as_any().downcast_ref::<Float64Array>().unwrap();
        let bytes_per_sec_array = batch.column(10).as_any().downcast_ref::<Float64Array>().unwrap();
        let protocol_array = batch.column(2).as_any().downcast_ref::<StringArray>().unwrap();
        let port_array = batch.column(3).as_any().downcast_ref::<Int64Array>().unwrap();
        
        let mut risk_levels = Vec::new();
        
        for i in 0..batch.num_rows() {
            if !is_spike_array.is_null(i) && !spike_severity_array.is_null(i) && 
               !bytes_per_sec_array.is_null(i) && !protocol_array.is_null(i) && !port_array.is_null(i) {
                ...
                let risk_level = Self::assess_risk_level(is_spike, spike_severity, bytes_per_sec, protocol, port);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📡 Generated network traffic data: {} events", traffic_events.len());
    ...
    while let Some(batch_result) = source.next_batch().await {
        ...
        println!("📊 Processing batch {} ({} events)", batch_count, batch.num_rows());
        
        // Apply transformation pipeline
        let transformed_batch = pipeline.apply(batch).await?;
        
        // Count spikes and critical risks
        if let Some(is_spike_array) = transformed_batch.column(14).as_any().downcast_ref::<BooleanArray>() {
            spike_count += is_spike_array.iter().filter(|&x| x == Some(true)).count();
        }
        
        if let Some(risk_level_array) = transformed_batch.column(17).as_any().downcast_ref::<StringArray>() {
            critical_count += risk_level_array.iter()
...
mod tests {
    ...
    fn test_risk_assessment() {
        ...
        let risk_low = RiskAssessmentCalculator::assess_risk_level(false, 0.0, 100000.0, "TCP", 8080);
        assert_eq!(risk_low, "MINIMAL");
        
        let risk_medium = RiskAssessmentCalculator::assess_risk_level(true, 100.0, 1500000.0, "UDP", 53);
