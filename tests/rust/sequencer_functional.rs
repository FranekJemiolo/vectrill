...
use vectrill::sequencer::{Sequencer, SequencerConfig, Ordering, LateDataPolicy};
...
fn test_late_data_handling() {
    ...
    // The late event (timestamp 500) should be dropped
    for i in 0..timestamps.len() {
        let current_timestamp = timestamps.value(i);
        assert_ne!(
            current_timestamp, 500,
            "Late event with timestamp 500 should be dropped"
        );
    }
    
    // Verify we have 4 events (late event dropped)
    assert_eq!(timestamps.len(), 4, "Should have 4 events after dropping late data");
}
...
fn test_key_based_ordering() {
    let config = SequencerConfig::new()
        .ordering(Ordering::ByKeyThenTimestamp)
        .batch_size(10)
        .max_lateness_ms(1000);
...
fn test_large_dataset_processing() {
    let config = SequencerConfig::new()
        .ordering(Ordering::ByTimestamp)
        .batch_size(15)  // Smaller batch size to test batching
        .max_lateness_ms(1000);
...
fn test_watermark_progression() {
    ...
    assert_eq!(sequencer.watermark(), 0, "Initial watermark should be 0");
...
fn test_flush_functionality() {
    ...
    // Should not auto-flush due to large batch size
    let auto_batch = sequencer.next_batch();
    assert!(auto_batch.is_none(), "Should not auto-flush with large batch size");
    
    // Force flush
    let flush_batch = sequencer.flush().expect("Should be able to flush");
    
    // Verify flushed batch has events
...
fn test_multiple_batch_ingestion() {
    ...
    // Verify we have all events from both batches
    assert_eq!(total_events, 12, "Should have all 12 events from both batches");
    ...
    for &timestamp in &all_timestamps {
        assert!(
            ...
            "Events should be globally ordered: {} >= {}",
            timestamp,
            prev_timestamp
        );
