...
async fn test_memory_to_file_csv() -> Result<(), VectrillError> {
    ...
    let mut sink = FileSink::new(temp_file.path().to_path_buf(), FileSinkFormat::Csv, schema)?;
...
async fn test_memory_to_file_json() -> Result<(), VectrillError> {
    ...
    // Create file sink
    let temp_file = NamedTempFile::new().unwrap();
    let mut sink = FileSink::new(temp_file.path().to_path_buf(), FileSinkFormat::Json, schema)?;
        ...
        Ok(mut sink) => {
            ...
            while let Some(batch_result) = source.next_batch().await {
                ...
                if write_result.is_err() {
                    println!(
                        "✅ Memory -> Kafka (JSON) test structure correct (Kafka not available)"
                    );
                    return Ok(());
                }
            }
    ...
    match kafka_result {
        Ok(mut sink) => {
            while let Some(batch_result) = source.next_batch().await {
                ...
                if write_result.is_err() {
                    println!(
                        "✅ Memory -> Kafka (CSV) test structure correct (Kafka not available)"
                    );
                    return Ok(());
                }
            }
...
async fn test_large_batch_processing() -> Result<(), VectrillError> {
    ...
    println!(
        "✅ Large batch processing test passed: {} rows in {:?}",
        total_rows, elapsed
    );
    Ok(())
}
...
async fn test_error_handling() -> Result<(), VectrillError> {
    ...
    let sink_result = FileSink::new(invalid_path, FileSinkFormat::Csv, schema);
