...
use vectrill::sequencer::{Ordering, Sequencer, SequencerConfig};
...
fn test_out_of_order_sequencing() {
    ...
    assert_eq!(
        timestamps.value(0),
        500,
        "First event should be timestamp 500"
    );
    ...
    assert_eq!(
        timestamps.value(5),
        3000,
        "Last event should be timestamp 3000"
    );
}
...
fn test_watermark_progression() {
    ...
    assert_eq!(
        sequencer.watermark(),
        i64::MIN,
        "Initial watermark should be i64::MIN"
    );
    ...
    assert!(
        sequencer.watermark() > 0,
        "Watermark should have advanced after processing"
    );
}
...
fn test_multiple_batch_ingestion() {
    ...
    sequencer
        .ingest(batch1)
        .expect("Failed to ingest first batch");
    sequencer
        .ingest(batch2)
        .expect("Failed to ingest second batch");
    ...
    assert!(
        total_events >= 10,
        "Should have at least 10 events from both batches"
    );
    ...
    assert_eq!(
        sorted_timestamps[0], expected_min,
        "Should have earliest non-late timestamp"
    );
    assert_eq!(
