...
use vectrill::expression::{Expr, ScalarValue, Operator};
use vectrill::memory::{BufferPool, global_buffer_pool};
use vectrill::performance::{Counter, CounterType, CounterRegistry, Timer, global_counter_registry};
...
fn test_buffer_pool_reuse() {
    ...
    // Get another - should reuse the buffer if capacity matches
    let arr2 = pool.get_array(&arrow::datatypes::DataType::Int64, 50);
    assert_eq!(arr2.len(), 50);
    
    // Check stats
    let stats = pool.stats();
    assert_eq!(stats.total_arrays, 0); // After getting again
}
...
#[test]
fn test_timer() {
    let counter = Counter::new(CounterType::TotalTimeUs);
