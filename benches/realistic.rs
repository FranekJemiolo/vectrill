...
use std::sync::Arc;
use vectrill::expression::{create_physical_expr, Expr, Operator as ExprOp, ScalarValue};
...
fn create_realistic_batch(size: usize) -> RecordBatch {
    ...
    let cats = StringArray::from_iter(
        (0..size).map(|i| Some(categories[(i * 7) % categories.len()].to_string())),
    );
    let values = Int64Array::from_iter((0..size).map(|i| Some(((i as i64 * 31) % 1000) + 100))); // Int64 values 100-1099
    let timestamps = Int64Array::from_iter(
        (0..size).map(|i| Some((i as i64) * 1000 + ((i as i64 * 37) % 1000))),
    );
